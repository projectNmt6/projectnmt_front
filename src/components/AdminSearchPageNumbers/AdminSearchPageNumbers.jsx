    ...
    return (
        <div css={s.layout}>
            <div css={s.pageNumbers}>
                {
                    parseInt(page) !== 1 &&
                    <Link css={s.pageButton(false)} to={`/admin/management/main?page=${page - 1}`}>&#60;</Link>
                }
                {
                    numbers.map(number => 
                        <Link key={number} css={s.pageButton(number === parseInt(page))} to={`/admin/management/main?page=${number}`}>{number}</Link>    
                    )
                }
                {
                    parseInt(page) !== maxPageNumber &&
                    <Link css={s.pageButton(false)} to={`/admin/management/main?page=${parseInt(page) + 1}`}>&#62;</Link>
                }
            </div>
            <div css={s.pageCount}>
